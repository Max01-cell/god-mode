...
function posSystemRouting(pos_system, best_merchant_savings) {
  ...
  if (/clover/.test(pos)) {
    return {
      pos_compatible: true,
      pos_locked: false,
      pos_recommendation: 'Merchant uses Clover — Beacon only. Beacon has native Clover integration ($5/mo platform fee); Beacon Flex Sell available at 50% split. Kurv uses MaxxPay (does not support Clover). EPI does not natively support Clover.',
  ...
  if (/square/.test(pos)) {
    return {
      ...
      pos_locked: true,
      pos_recommendation: 'Merchant on Square — must switch hardware. Hardware options: Beacon (free Clover via Flex Sell if volume qualifies), EPI (free Exatouch), Kurv (MaxxPay). Highlight savings from eliminating Square flat rate (2.6% + $0.10) vs interchange-plus.',
  ...
  if (/toast/.test(pos)) {
    ...
    return {
      pos_compatible: false,
      pos_locked: true,
      pos_recommendation: 'Merchant on Toast — must switch hardware. Savings justify switch. Hardware options: Beacon (Clover via Flex Sell), EPI (Exatouch), Kurv (MaxxPay).',
      equipment_action: 'replace',
      negotiate_existing: false,
      preferred_processor: 'Beacon Payments',
      deal_difficulty_factor: 'medium',
  ...
  if (/heartland|genius/.test(pos)) {
    ...
    return {
      pos_compatible: false,
      pos_locked: true,
      pos_recommendation: 'Merchant on Heartland Genius — locked system. Savings exceed $500/mo threshold — recommend full POS switch. Hardware options: Beacon (Clover via Flex Sell), EPI (Exatouch), Kurv (MaxxPay).',
      equipment_action: 'replace',
      negotiate_existing: false,
      preferred_processor: 'Beacon Payments',
      deal_difficulty_factor: 'hard',
  ...
  if (/spoton/.test(pos)) {
    ...
    return {
      pos_compatible: false,
      pos_locked: true,
      pos_recommendation: 'Merchant on SpotOn — must switch hardware. Savings justify switch. Hardware options: Beacon (Clover via Flex Sell), EPI (Exatouch), Kurv (MaxxPay).',
      equipment_action: 'replace',
      negotiate_existing: false,
      preferred_processor: null,
      deal_difficulty_factor: 'medium',
  ...
  if (/shopify|lightspeed|revel/.test(pos)) {
    return {
      pos_compatible: false,
      pos_locked: true,
      pos_recommendation: `Merchant on ${pos_system} — bundled POS system, processing tied to software. More involved to switch. Show the numbers and let savings make the case. If switching, hardware options: Beacon (Clover via Flex Sell), EPI (Exatouch), Kurv (MaxxPay).`,
      equipment_action: 'replace',
      negotiate_existing: false,
      preferred_processor: null,
      deal_difficulty_factor: 'medium',
